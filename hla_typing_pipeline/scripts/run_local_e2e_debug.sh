#!/usr/bin/env bash
# Local reduced-E2E debug runner for the four Puhti-problematic BAM tools.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_DIR="$(dirname "$SCRIPT_DIR")"
DEFAULT_BAM="/home/umut/hla_calibration/hla_bams/NA19238.bam"
TOOLS="hlahd,spechla,arcashla,optitype"
STAMP="$(date +%Y%m%d_%H%M%S)"

INPUT_BAM="${1:-$DEFAULT_BAM}"
RUN_NAME="${2:-local_e2e_debug_${STAMP}}"
OUTDIR="${PIPELINE_DIR}/${RUN_NAME}"
WORKDIR="${PIPELINE_DIR}/work_${RUN_NAME}"
USER_CONFIG="${PIPELINE_DIR}/conf/user.config"
DEBUG_CONFIG="${PIPELINE_DIR}/conf/local_debug_e2e.config"
RUN_CONFIG="${PIPELINE_DIR}/run.config"

require_file() {
    local path="$1"
    if [[ ! -f "$path" ]]; then
        echo "ERROR: required file not found: $path" >&2
        exit 1
    fi
}

require_dir() {
    local path="$1"
    if [[ ! -d "$path" ]]; then
        echo "ERROR: required directory not found: $path" >&2
        exit 1
    fi
}

if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    cat <<EOF
Usage: $0 [input_bam] [run_name]

Runs one local reduced-E2E debug pass with:
  ${TOOLS}

Defaults:
  input_bam = ${DEFAULT_BAM}
  run_name  = ${RUN_NAME}

Output:
  ${PIPELINE_DIR}/<run_name>
  ${PIPELINE_DIR}/work_<run_name>
EOF
    exit 0
fi

require_file "$INPUT_BAM"
require_file "${INPUT_BAM}.bai"
require_file "$USER_CONFIG"
require_file "$DEBUG_CONFIG"
require_file "$RUN_CONFIG"

command -v nextflow >/dev/null || { echo "ERROR: nextflow not found in PATH" >&2; exit 1; }
command -v singularity >/dev/null || { echo "ERROR: singularity not found in PATH" >&2; exit 1; }
command -v samtools >/dev/null || { echo "ERROR: samtools not found in PATH" >&2; exit 1; }

require_dir "/home/umut/SpecHLA"
require_dir "/home/umut/miniconda3/envs/arcas-hla"
require_file "/home/umut/bin/samtools_wrapper.sh"
require_file "/home/umut/projects/project_2008084/ozcanumu/hla_rnaseq_analysis/hla_references/containers/hlahd.sif"
require_file "/home/umut/projects/project_2008084/ozcanumu/hla_rnaseq_analysis/hla_references/containers/arcashla.sif"
require_file "/home/umut/projects/project_2008084/ozcanumu/hla_rnaseq_analysis/hla_references/containers/optitype.sif"

mkdir -p "$OUTDIR"

echo "==================================================================="
echo " Local Reduced-E2E Debug"
echo "==================================================================="
echo "Input BAM : $INPUT_BAM"
echo "Run name  : $RUN_NAME"
echo "Output    : $OUTDIR"
echo "Work dir  : $WORKDIR"
echo "Tools     : $TOOLS"
echo "Configs   : conf/user.config + run.config + conf/local_debug_e2e.config"
echo "==================================================================="

cd "$PIPELINE_DIR"

set +e
nextflow run main.nf \
    -c "$USER_CONFIG" \
    -c "$RUN_CONFIG" \
    -c "$DEBUG_CONFIG" \
    -profile singularity \
    -work-dir "$WORKDIR" \
    --input_bam "$INPUT_BAM" \
    --outdir "$OUTDIR" \
    --tools "$TOOLS"
nf_exit=$?
set -e

echo
echo "==================================================================="
echo " Run Summary"
echo "==================================================================="
echo "Nextflow exit code: $nf_exit"
for tool in hlahd spechla arcashla optitype; do
    result_file="$(find "$OUTDIR" -path "*/${tool}/*_${tool}.txt" -type f | head -n 1 || true)"
    if [[ -n "$result_file" ]]; then
        echo "[OK] ${tool}: ${result_file}"
    else
        echo "[WARN] ${tool}: no published result file"
    fi
done

TRACE_FILE="$(find "$OUTDIR/pipeline_info" -maxdepth 1 -name 'trace_*.txt' -type f | sort | tail -n 1 || true)"
if [[ -n "$TRACE_FILE" ]]; then
    echo
    echo "Latest trace: $TRACE_FILE"
    awk -F'\t' 'NR==1 || $4 ~ /SPECHLA|HLAHD|ARCASHLA|OPTITYPE/' "$TRACE_FILE"

    echo
    echo "Per-tool task diagnostics:"
    for proc in SPECHLA HLAHD ARCASHLA OPTITYPE; do
        work_rel="$(awk -F'\t' -v proc="$proc" 'NR>1 && $4 == proc {print $3}' "$TRACE_FILE" | tail -n 1)"
        if [[ -z "$work_rel" ]]; then
            echo "[WARN] ${proc}: no task row found in trace"
            continue
        fi

        work_path="${WORKDIR}/${work_rel}"
        echo
        echo "=== ${proc} ==="
        echo "Work dir: ${work_path}"
        if [[ -d "$work_path" ]]; then
            if [[ -f "${work_path}/.exitcode" ]]; then
                echo "Exit code: $(cat "${work_path}/.exitcode")"
            fi
            for f in .command.sh .command.out .command.err; do
                if [[ -f "${work_path}/${f}" ]]; then
                    echo "--- ${f} ---"
                    sed -n '1,120p' "${work_path}/${f}"
                fi
            done
        else
            echo "[WARN] work dir not found"
        fi
    done
else
    echo "[WARN] No trace file found under $OUTDIR/pipeline_info"
fi

exit "$nf_exit"
